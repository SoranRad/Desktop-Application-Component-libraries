...
namespace MS_Control.TSDD
{
    ...
    public class CopyEventHandlers
    {
        ...
        /// <returns></returns>
        public Dictionary<string, Dictionary<object, Delegate[]>> GetHandlersFrom(object ctrl)
        {
        ...
        private string GetEventNameFromKey(object ctrl, object key)
        {

            foreach (var eventKey in KeyEventNameMapping.Keys)
            {
                var fieldInfo = typeof(object).GetField(eventKey, BF.GetField | BF.Static | BF.NonPublic | BF.DeclaredOnly);
                if (fieldInfo != null)
                {
